...
public class AdminAnalyticsServiceImpl implements AdminAnalyticsService {
    ...
    public SummaryCardsRs getOverview(LocalDate startDate, LocalDate endDate) {
        ...
        Long prevRevenue = orderItemRepo.getTotalRevenue(prevStart, start);
        int salesTrend = calculatePercentChange(prevRevenue, revenue);

        // Get top selling product
        var topList = orderItemRepo.getTopSellingProduct(start, end);
        String topProduct = (topList != null && !topList.isEmpty())
                ? topList.get(0)[0].toString()
                : "N/A";
        ...
        return mapper.toSummaryCards(
                ...
                topProduct != null ? topProduct : "N/A"
        );
    }
    ...
    private List<WeeklyDataPoint> generateWeeklyRevenue(LocalDateTime start, LocalDateTime end) {
        ...
        long days = ChronoUnit.DAYS.between(start, end);
        int weeks = (int) Math.min(days / 7, 12); // Max 12 weeks
        ...
        for (int i = 0; i < weeks; i++) {
            LocalDateTime weekEnd = currentStart.plusWeeks(1);
            if (weekEnd.isAfter(end)) weekEnd = end;

            // Get revenue for this week
            Long revenue = orderItemRepo.getTotalRevenue(currentStart, weekEnd);

            // Format week label
            String weekLabel = String.format("Week %d", i + 1);
    ...
    private List<WeeklyDataPoint> generateWeeklyActivity(LocalDateTime start, LocalDateTime end) {
        ...
        for (int i = 0; i < weeks; i++) {
            ...
            // Count active users for this week (users who placed orders)
            Long activeUsers = orderRepo.getActiveUsers(currentStart, weekEnd);
